...
import type { LocalForage, ProxyStorage, ExpiresData } from './types.d.ts';
	...
	public async getItem<T>(k: string): Promise<T> {
		return new Promise((resolve, reject) => {
			this.storage
				...
				.then((value) => {
					value && (value.expires > new Date().getTime() || value.expires === 0) ? resolve(value.data) : resolve(null as any);
				})
				.catch((err) => {
					reject(err);
				});
